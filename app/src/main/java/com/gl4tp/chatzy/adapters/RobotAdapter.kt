...
import com.gl4tp.chatzy.databinding.ViewRobotLayoutBinding
...
class RobotAdapter (
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.viewRobotLayoutBinding.deleteImg.setOnClickListener(){
            if (holder.adapterPosition != -1){
                onClickDeleteUpdateCallback("delete",holder.adapterPosition,robot)
            }
        }
        holder.viewRobotLayoutBinding.editImg.setOnClickListener(){
            if (holder.adapterPosition != -1){
